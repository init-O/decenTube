...
contract('DVideo', ([deployer, author]) => {
  ...
  describe('videos', async () => {
    ...
    it("should like Video", async () => {
      ...
      const tipper = accounts[0]
      const _video = await dvideo.videos(videoCount)
      assert.notEqual(_video.author, tipper)
      const initalLikeCount = _video.like_count
      let oldAuthorBalance 
      oldAuthorBalance = await web3.eth.getBalance(_video.author)
      oldAuthorBalance = new web3.utils.BN(oldAuthorBalance)

      const video = await dvideo.likeVideo(videoCount,{from: tipper, value: web3.utils.toWei('1', 'Ether') })

      const like_event = video.logs[0].args
      assert(like_event.id.toNumber(),videoCount.toNumber(),"id is correct")
      assert(like_event.hash, _video.hash,"correct hash")
      assert(like_event.title,_video.title,"correct title")
      assert(like_event.like_count, initalLikeCount+1,"like count increased")

      let newAuthorBalance 
      newAuthorBalance = await web3.eth.getBalance(_video.author)
      newAuthorBalance = new web3.utils.BN(newAuthorBalance)

      let amount;
      amount = await web3.utils.toWei("1", "ether")
      amount = new web3.utils.BN(amount)

      const expectedBalance = oldAuthorBalance.add(amount)

      assert(expectedBalance, newAuthorBalance, "balance must be equal")
    })
  })
})